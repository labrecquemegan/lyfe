...
export default function UserMeters({ user }) {
  ...
  const mindfulPercentage =
    (user.mindfulness_stats.todays_mindful_duration / user.mindful_goal) * 100;

  const exercisePercentage =
    (user.exercise_stats.todays_exercise_duration / user.exercise_goal) * 100;
        ...
        <div className="meter">
          ...
          >
            <div style={{ width: "82%" }}>
              {/* MINDFULNESS RING */}
              <CircularProgressbarWithChildren
                value={mindfulPercentage === 0 ? 1 : mindfulPercentage}
                strokeWidth={9}
                styles={buildStyles({
                  pathColor: "#9f3ba3",
                  trailColor: "#e7e7e7",
              ...
              >
                <div style={{ width: "79.25%" }}>
                  {/* EXERCISE RING */}
                  <CircularProgressbarWithChildren
                    value={exercisePercentage === 0 ? 1 : exercisePercentage}
                    strokeWidth={10}
                    styles={buildStyles({
                      pathColor: "#ffae18",
                      trailColor: "#e7e7e7",
